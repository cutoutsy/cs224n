...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    d = np.apply_along_axis(lambda x : np.sqrt(x.T.dot(x)), 1, x)
    x /= d[:, None] 
    ### END YOUR CODE
    return x
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    predicted = inputVectors[cword_idx]

    for j in contextWords:
        index = tokens[j]
        c_cost, c_grad_in, c_grad_out = word2vecCostAndGradient(predicted, index, outputVectors, dataset)
        cost += c_cost
